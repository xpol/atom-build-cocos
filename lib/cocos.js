...
const addSimulatorTask = (builder, tasks, errorMatch) => {
  ...
  // When running in shell mode, quoted path is auto trimmed before passed to simulator.
...
const setupAndroidArgs = (builder, plat, args) => {
  if (plat === 'android') {
    if (builder.androidStudio) args.push('--android-studio');
    if (builder.androidABI) args.push('--app-abi', builder.androidABI);
  }
};
...
const addReleaseTask = (builder, tasks, exec, plat, errorMatch) => {
  ...
  if (plat === 'ios') {
    if (builder.iosCodeSignIdentity.length > 0) args.push('--sign-identity', builder.iosCodeSignIdentity);
  }
  setupAndroidArgs(builder, plat, args);
  tasks.push({
...
export const config = {
  consoleRoot: {
    ...
    description: 'Absolute path to the global `cocos-console` directory. eg `/Users/xpol/Workspace/cocos2d-x/tools/cocos2d-console`',
    type: 'string',
    default: '',
    order: 1
  },
  androidABI: {
    title: 'Android ABI',
    description: 'Set the APP_ABI of ndk-build. Can overwrite in `.cocos-project.json` with root key `androidABI`.',
    type: 'string',
    default: 'armeabi',
    order: 5
  },
