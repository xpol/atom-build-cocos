...
const getExecutable = (cwd, console_root) => {
  ...
  const globalPath = path.join(console_root, "bin", executable);
...
export function provideBuilder() {
  return class MakeBuildProvider {
    ...
    settings() {
      const exec = getExecutable(this.cwd, this.ccconf.console_root);
      ...
      for (const platform in supports) {
        ...
        for (let i = 0; i < pconf.length; i++) {
          const cmd = pconf[i];
          config = addTask(config, exec, cmd, platform, regexs);
        }
      }
      return config;
    }
